...
import { Manga, getMangaData, getChaptersData, isPageAPI } from '../../pages/AnimeLib/api';
...
export const HentaiLib: pageInterface = {
  name: 'HentaiLib',
  domain: ['https://hentailib.me'],
  languages: ['Russian'],
  type: 'manga',
  getImage() {
  ...
  sync: {
    ...
    getOverviewUrl(url) {
      return utils.absoluteLink(`ru/manga/${manga.data.slug_url}`, HentaiLib.domain);
    },
    getEpisode(url) {
  ...
  init(page: SyncPage) {
    ...
    async function check() {
      ...
      if (
        !HentaiLib.isSyncPage(window.location.href) &&
        !HentaiLib.isOverviewPage!(window.location.href)
      )
        return;

      // NOTE - if we are on sync page
      if (HentaiLib.isSyncPage(window.location.href)) {
        await updateSyncPage();
        await awaitReaderLoading();
      }

      // NOTE - if we are on overview page
      if (HentaiLib.isOverviewPage!(window.location.href)) {
...
async function updateSyncPage() {
  ...
  if (chaptersData) {
    ...
    manga.reader.next = utils.absoluteLink(nextButton.attr('href'), HentaiLib.domain);
    if (data) {
